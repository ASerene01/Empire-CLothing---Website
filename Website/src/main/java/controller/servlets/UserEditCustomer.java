...
@WebServlet(asyncSupported = true, urlPatterns = { "/UserEditCustomer" })
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
	maxFileSize = 1024 * 1024 * 10, // 10MB
	maxRequestSize = 1024 * 1024 * 50)
public class UserEditCustomer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		EditUserCustomer userModel = new EditUserCustomer(userId, firstName, lastName, username, encryptedPassword, email, phoneNumber, userType);
		...
		if(result == 1) {
			...
			request.getRequestDispatcher("/View/Pages/EditUserWithPicture.jsp").forward(request, response);
		}
