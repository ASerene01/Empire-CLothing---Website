...
@WebServlet(asyncSupported = true, urlPatterns = { "/ProfileUpdateAdminPicture" })
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
	maxFileSize = 1024 * 1024 * 10, // 10MB
	maxRequestSize = 1024 * 1024 * 50)
public class ProfileUpdateAdminPicture extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result == 1) {
			...
			request.getRequestDispatcher("/View/Pages/ProfileUpdateAdminPicture.jsp").forward(request, response);
		}
	}
